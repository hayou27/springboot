...
@Repository // 어노테이션 안 써도 됨 -> extends JpaRepositoty를 해 주어서!
public interface UserRepository extends JpaRepository<UserEntity, Long>{
	
	// 1. 인터페이스 상속 받기
	// 인터페이스간 상속은 extends 키워드 사용
	
	// <> 제너릭 : 레퍼런스 타입만 가능 / 객체 타입을 지정해 줌-> 기본타입 8가지는 들어갈 수 없습니다
	// --> Wrapper Class : 기본 타입 8가지를 레퍼런스 타입으로 사용할 수 있게 해준 class 
	
	// T : entity(VO) 객체 --> class 파일로 구성 시 테이블 생성(테이블을 VO 객체로 만들어줌)
	// ID : entity가 가지고 있는 PK필드의 타입(@Id가 걸려있는 필드의 타입)
	 ...
	 * 5. 커스텀 메소드
	  ...
	  규칙 : ★★★ 단어사이가 이어질 시 무조건 카멜기법 ★★★
	  		-> select는 find
	   		-> where 는 by
	   		-> 컬럼명 작성
	   		-> and, or, order by등 키워드가 있을 시 모두 작성
	  -> findByUserIdAndPw(매개변수1, 매개변수2)
	  
	 * 6. existsBy컬럼명(매개변수) : 데이터 유무를 알려주는 함수
	  -> select count ~ limit
	  -> 기본 : select * from 테이블명 where 컬럼 = 매개변수
	*/
	
	
	// userId가 있는지 없는지
	// existsByUserId라는 함수는 없다
	// --> 커스텀으로 만들어야 함!
